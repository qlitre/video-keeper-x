...
export default function VideoEditForm({ video, artists = [] }: VideoEditFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    x_account_id: video.x_account_id || '',
    venue: video.venue || '',
    event_date: video.event_date || '',
    song_name: video.song_name || ''
  })

  // 初期値として動画データから選択されたアーティストを設定
  useEffect(() => {
    if (video.artist_id && artists.length > 0) {
      const artist = artists.find(a => a.id === video.artist_id)
      if (artist) {
        setSelectedArtist(artist)
      }
    }
  }, [video.artist_id, artists])
    ...
    if (searchQuery.trim() === '') {
      ...
      const filtered = artists.filter(
        artist =>
          ...
          (artist.name_kana &&
            artist.name_kana.toLowerCase().includes(searchQuery.toLowerCase()))
      )
  ...
  return (
    <>
      <form action={`/videos/edit/${video.id}`} method="post" class="space-y-6">
        <div>
          <label
            for="video_url"
        ...
        <div class="relative">
          ...
          <input
            type="hidden"
            name="artist_id"
            value={selectedArtist?.id || video.artist_id || ''}
          />
        </div>
                    ...
                    <div class="max-h-96 overflow-y-auto">
                      {filteredArtists.length === 0 ? (
                        <div class="text-center py-8">
                          ...
                          <a
                            href="/artists/add?return_to=video_edit"
